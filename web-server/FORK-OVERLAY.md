...
| Locked repository/worktree navigation in embeds | `getEmbeddedParentOrigin`, `WebApp`, and `DesktopToolbar` in `app/src/ui/web-app.tsx`, assertions in `web-server/test-source-dependencies.js` | A valid `parentOrigin` marks the Vibing embedded mode. It hides `RepositoryToolbarDropdown` and `WorktreeDropdown`, so the host-selected `?repository=` worktree cannot be changed within the iframe; switch via Vibing workspace tabs instead. A standalone browser tab without `parentOrigin` retains both controls. |
...
2. Embed `http://127.0.0.1:9611/?repository=<absolute worktree path>&parentOrigin=<host origin>`
   ...
   row is visible and every button clickable, repository/worktree pickers are
   absent, the Branch foldout opens fully inside the viewport, and the dark
