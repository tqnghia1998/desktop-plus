...
| Embed type scale | `:root` font-size block in `web-server/src/browser-renderer.css`; diff default in `app/src/ui/web-app.tsx` | The web renderer bumps every `_variables.scss` type-scale variable by 1px (`--font-size` 12→13px, sm 11→12px, md 14→15px, lg 28→29px, xl 32→33px, xxl 42→43px, xs 9→10px) because the 12px base reads too small inside the Space App Vibing left-tab iframe. The diff editor gets a separate +2px default: the web state initializer reads `getNumber('diff-font-size', defaultDiffFontSize + 2)` (13px, `app/src/models/diff-font.ts` keeps the shared desktop default). Web-only: the desktop app keeps the upstream scale and diff default. |
| Vibing runtime bundle | `scripts/bundle-desktop-plus-web.mjs` (+ `bundle:vibing` script) | `yarn build:web && yarn bundle:vibing` writes a self-contained runtime into `../space-app-vibing/scripts/desktop-plus-web/` (committed build output there, shipped inside the vibing app). It copies the require closure of `web-server/server.js` (walks relative + bare requires), `web-server/public/`, `ssh-askpass.js` (spawned by path, never required), the runtime packages (dugite with embedded git, keytar, ignore, semver and transitive deps), and a `package.json` pinning `type: "commonjs"` because vibing's root package.json is ESM. The output carries platform-specific binaries, so each release OS must re-run the bundler from a checkout on that OS. |
