...
| Worktree family in the repository picker | `addRepositoryWithWorktrees` in `app/src/ui/web/contracts.ts` and `app/src/ui/web/store.ts`, called by the `?repository=` effect in `app/src/ui/web-app.tsx` | Opening the app with `?repository=<path>` adds the requested repository **plus every sibling worktree** as its own flat repository row, without selecting the siblings; selection stays on the requested path. Siblings are persisted with `lastOpenedAt: 0` and null branches. Relies on the server computing worktrees only for the selected repository (`git worktree list`, main worktree first), so repositories selected before this change gain sibling rows on the next `?repository=` mount. |
| Locked repository/worktree navigation in embeds | `getEmbeddedParentOrigin`, `WebApp`, and `DesktopToolbar` in `app/src/ui/web-app.tsx`, plus the `disabled` prop in `app/src/ui/toolbar/worktree-dropdown.tsx` and assertions in `web-server/test-source-dependencies.js` | A valid `parentOrigin` marks the Vibing embedded mode. It keeps `RepositoryToolbarDropdown` and `WorktreeDropdown` visible but disabled, so the host-selected `?repository=` worktree cannot be changed within the iframe; switch via Vibing workspace tabs instead. A standalone browser tab without `parentOrigin` keeps both controls enabled. |
...
| Space App Vibing dark theme, typography, and spacing | `:root`, `dialog`, and `body.theme-dark` override blocks in `web-server/src/browser-renderer.css`, plus the web default `diff-font-size` in `app/src/ui/web-app.tsx` (load after `app/styles/web-desktop.scss` and win the cascade) | Maps the GitHub dark palette onto the Space App Vibing Metro tokens: `#1a1a1a` surfaces, `#2d2d30` hover, `#3f3f46` selected/badges, `#252526` secondary buttons/menu panes/tooltips, `#222225`/`#2c2c30` borders, `#ffffff`/`#a0a0a0` text, `#3a96dd` accent (`#55a8e3` hover), plus the skeleton and box-edge gradients rebuilt around `rgba(26, 26, 26, …)`. The semantic type tokens stay 1px larger than desktop defaults and shared `--spacing` remains 10px. Embedded dialogs use a 600px minimum width instead of the desktop 400px floor (the shared responsive rule still fits them below 600px). Web-only controls use 32px buttons, inputs, split-dropdowns, and menu rows, with a 58px toolbar, 36px tabs, 8px radii, 32px branch/worktree dropdown rows, and 50px History commit rows. Embedded Changes, commit-file, and stash-file rows use 32px, stash headers use the normal `--font-size-md` title, one-spacing vertical padding, and a half-spacing title/action gap rather than desktop's `--font-size-xl` title, triple-spacing padding, and full-spacing gaps, and the default diff editor font is 12px (2px below the prior embedded default). The last Branch action button has no trailing border. Web-only: the desktop app is untouched. |
| Browser CSS ownership guard | Expected `browser-renderer.css` content and failure message in `web-server/test-ui-ownership.js` | The guard pins the exact browser-renderer.css content; its policy is that browser CSS may only size the renderer root and shared dialog/menu surfaces, align the dark theme and type scale with the Space App Vibing host, and fit the toolbar in narrow viewports. After any `browser-renderer.css` change, regenerate the expected string from the file. |
...
| Host-triggered state refresh | The `space:desktop-plus-refresh-request` handler in `app/src/ui/web-app.tsx`, guarded by `web-server/test-source-dependencies.js` | A matching trusted parent message refreshes repository data only when Desktop Plus is idle; it never queues a deferred refresh. Queued refreshes run after a user action has opened a modal, replacing its data and dismissing the modal. The Vibing host sends the message after its iframe loads and after a confirmed Git state change—not on a timer. |
| Vibing runtime bundle | `scripts/bundle-desktop-plus-web.mjs` (+ `bundle:vibing` script) | `yarn build:web && yarn bundle:vibing` writes a self-contained runtime into `../space-app-vibing/scripts/desktop-plus-web/` (committed build output there, shipped inside the vibing app). It copies the require closure of `web-server/server.js` (walks relative + bare requires), `web-server/public/`, `ssh-askpass.js` (spawned by path, never required), and a `package.json` pinning `type: "commonjs"` because vibing's root package.json is ESM. Runtime packages (dugite with embedded git, keytar, ignore, semver and transitive deps) are committed as `runtime-node-modules.tgz`, since Vibing ignores `node_modules`; bare server requires must also resolve from the bundle-root `node_modules`. Directory copying preserves relative symlinks in dugite's git-core, so the archive can move between machines. Vibing packaging expands the archive into the staged server bundle. On macOS the bundler prunes dugite's `git/libexec/git-core` of git-lfs and the .NET runtime behind Git Credential Manager (~120MB): credential flows re-point `GIT_EXEC_PATH` at the system Git (only it carries the osxkeychain helper), and Vibing hosts have no git-lfs. Windows keeps GCM and must not be pruned. The archive carries platform-specific binaries, so each release OS must regenerate it from the pinned committed fork revision via Vibing's `scripts/prepare-desktop-plus-runtime.mjs`; CI does not rely on uncommitted fork edits. |
...
2. Embed `http://127.0.0.1:9611/?repository=<absolute worktree path>&parentOrigin=<host origin>`
   in a ~900px-wide iframe next to a host panel and check: the whole toolbar
   row is visible and every enabled button clickable, repository/worktree
   pickers are visible but disabled, the Branch foldout opens fully inside the viewport, and the dark
   theme matches the Space App Vibing palette.
