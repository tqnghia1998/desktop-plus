...
| Embed type scale | `:root` font-size block in `web-server/src/browser-renderer.css` | The web renderer bumps every `_variables.scss` type-scale variable by 2px (`--font-size` 12→14px, sm 11→13px, md 14→16px, lg 28→30px, xl 32→34px, xxl 42→44px, xs 9→11px) because the 12px base reads too small inside the Space App Vibing left-tab iframe. Web-only: the desktop app keeps the upstream scale. The diff font size is separate (persisted `diff-font-size` setting, default 11px in `app/src/models/diff-font.ts`) and is not part of this block. |
