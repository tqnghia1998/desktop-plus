...
| Space App Vibing dark theme, typography, and spacing | `:root`, `dialog`, and `body.theme-dark` override blocks in `web-server/src/browser-renderer.css`, plus the web default `diff-font-size` in `app/src/ui/web-app.tsx` (load after `app/styles/web-desktop.scss` and win the cascade) | Maps the GitHub dark palette onto the Space App Vibing Metro tokens: `#1a1a1a` surfaces, `#2d2d30` hover, `#3f3f46` selected/badges, `#252526` secondary buttons/menu panes/tooltips, `#222225`/`#2c2c30` borders, `#ffffff`/`#a0a0a0` text, `#3a96dd` accent (`#55a8e3` hover), plus the skeleton and box-edge gradients rebuilt around `rgba(26, 26, 26, …)`. The semantic type tokens and default diff font are each 3px larger than the desktop defaults; shared `--spacing` remains the desktop 10px so adjacent actions and modal padding remain compact. Embedded dialogs use a 600px minimum width instead of the desktop 400px floor (the shared responsive rule still fits them below 600px). Web-only controls use 40px button/input heights (instead of 25px), split-dropdown and menu items use 42px rows (instead of 30px), the toolbar and tabs are 64px and 40px tall (instead of 50px and 29px), box/control radii are 10px (rather than 6px), and History uses 64px commit rows (instead of 50px), so dense boxes, dropdowns, forms, lists, dialogs, and the history list remain comfortable in the compact host iframe. Web-only: the desktop app is untouched. |
